...
(*                                                                            *)
(* Today it does not. The journal and the transcript are durable and the CELL  *)
(* is not: when the daemon stops, every live session goes with it, and nothing *)
(* brings one back. A person reconstructs one by hand from the picker, which   *)
(* starts a NEW session that happens to hold the same conversation.            *)
(*                                                                            *)
...
(*                                                                            *)
(* IDENTITY. A session is named by an in-memory counter, `s1`, `s2`, which     *)
(* restarts at zero with the daemon. Rehydrating cells without fixing that is  *)
(* worse than losing them: every client holding `s1` would go on addressing    *)
(* `s1`, and `s1` would be a different conversation. DURABLE = TRUE models a   *)
(* name minted from the record, which no restart can mint again.               *)
