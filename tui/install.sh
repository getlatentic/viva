#!/bin/sh
# Build viva-tui and link it next to `viva` on your PATH.
#
#     sh tui/install.sh
#
...
command -v cargo >/dev/null 2>&1 || {
  echo "viva-tui needs a Rust toolchain, and there is no cargo on your PATH." >&2
...
# NOT linked onto the PATH under its own name. `viva` is the command; the
...
echo "== built"
echo "  $here/target/release/viva-tui"
echo
