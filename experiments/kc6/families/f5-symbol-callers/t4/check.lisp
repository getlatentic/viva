...
(defvar *expected* (let ((zeros (sort (loop for sym being the external-symbols of (find-package "MINI.LEDGER")
                         when (zerop (total-calls sym))
                           collect (string-downcase (symbol-name sym)))
                   #'string<)))
  (format nil "~{~a~^~%~}" zeros)))
